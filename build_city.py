...
line_objects = set()
for line in lines:
    id_list = list(station_order[line])
    connections = set()
    for ind, station_id in enumerate(id_list[:-1]):
        station1 = {station for station in stations if station.network_id == station_id}
        station2 = {station for station in stations if station.network_id == id_list[ind+1]}
...
output = open(f"{city}_network.pkl", "wb+")
